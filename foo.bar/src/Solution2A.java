...
public class Solution2A {
    private static int parent(int h, int node, int root, int value) {
        if (node == value) return root;
        int left = (1 << (h - 1)) - 1;
        if (node - left <= value) {
            return parent(h - 1, node - 1, node, value);
        } else {
            return parent(h - 1, node - left - 1, node, value);
        }
    }
