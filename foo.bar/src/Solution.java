...
public class Solution {
    ...
    public static BigInteger solve(BigInteger a, BigInteger b) {
        ...
        BigInteger dd = a.mod(d);
        if (dd.equals(BigInteger.ZERO)) {
            return a.divide(d);
        }
        BigInteger x = a.divide(d);
        BigInteger ans = solve(dd, d);
        ans = ans.add(x);
        ans = ans.add(one);
        return ans;
    }
    ...
    public static String solution(String x, String y) {
        ...
        BigInteger gcd = maxx.gcd(minn);
        if (gcd.compareTo(one) != 0 || maxx.compareTo(zero) == 0 || minn.compareTo(zero) == 0) {
            return "impossible";
        } else {
            return solve(minn, maxx).toString();
        }
    }
}